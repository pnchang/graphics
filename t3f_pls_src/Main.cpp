...
#define	WIDTH	640
#define	HEIGHT	480
#define	ISWINDOWED	true	// FullScreen/Windowed
...
LPDIRECT3DTEXTURE9	Particle;
LPDIRECT3DTEXTURE9	Font;
LPDIRECT3DTEXTURE9	Masker;
...
LRESULT CALLBACK WndProc(HWND wnd, UINT msg, WPARAM w, LPARAM l)
{
	...
	case WM_CLOSE:
   	...
   	case WM_KEYDOWN:         // Is A Key Being Held Down?
      		...
      		{
      		case VK_ESCAPE:
			running = false;	// PostQuitMessage(0);   // post quit message
			break;
		}
	default:
		break;
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmd, int zooi)
{
	...
	if (D3Ddevice->CreateVertexBuffer(2197*sizeof(VERTEX),0,FVF_VERTEX,D3DPOOL_DEFAULT,&Verts,NULL) != D3D_OK) return 0;

	// create some textures
	if (D3Ddevice->CreateTexture(8,8,1,0,D3DFMT_A8R8G8B8,D3DPOOL_MANAGED,&Particle,NULL) != D3D_OK) return 0;
	if (D3Ddevice->CreateTexture(512,16,1,0,D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &Font, NULL) != D3D_OK) return 0;
	...
	VERTEX* vbuf;
	int index = 0;
	Verts->Lock(0, sizeof(VERTEX)*2197, (void**)&vbuf, 0);
	for (int z = -6; z <= 6; z++)
	{
		for (int y = -6; y <= 6; y++)
		{
	...
	SquareVerts[2].y = HEIGHT;
	SquareVerts[3].y = HEIGHT;
	...
	while(running)
	{
		...
		else
		{
			...
			if (currenttime < 32000)
			{
				...
				D3Ddevice->SetStreamSource(0, Verts, 0, sizeof(VERTEX));
			...
			{
				int x = HEIGHT+20 - ((currenttime-32000) / 40);
